...
module.exports = class DiffView {
  ...
  /**
   * Sets up listeners for soft-wrap changes on both editors.
   * When soft-wrap settings change, we update offset decoration heights.
   */
   ...
   * Updates offset decoration heights to maintain proper alignment.
   */
  _onSoftWrapChanged() {
    this._editorDiffExtender1.updateOffsetHeights();
    this._editorDiffExtender2.updateOffsetHeights();
  }
  ...
  _scrollToPositionAtFraction(editor, bufferRow, fraction) {
    ...
    const targetPixelPosition = editor.screenPositionForBufferPosition([bufferRow, 0]).row * lineHeight;
