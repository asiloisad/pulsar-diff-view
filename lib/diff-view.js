...
module.exports = {
  ...
  activate(state) {
    ...
    styleCalculator.startWatching(
      ...
      ['diff-view.colors.addedColor', 'diff-view.colors.removedColor'],
      (config) => {
        const addedColor = config.get('diff-view.colors.addedColor');
        addedColor.alpha = 0.4;
        const addedWordColor = addedColor;
        addedWordColor.alpha = 0.5;
        const removedColor = config.get('diff-view.colors.removedColor');
...
.diff-view-word-removed-custom .region {
  ...
  _resumeUpdateDiff(editors, computedDiff) {
    ...
    const addedColorSide = this.options.addedColorSide != null ? this.options.addedColorSide : this._getConfig('colors.addedColorSide');
    const diffWords = this.options.diffWords != null ? this.options.diffWords : this._getConfig('diffWords');
    const ignoreWhitespace = this.options.ignoreWhitespace != null ? this.options.ignoreWhitespace : this._getConfig('ignoreWhitespace');
    const overrideThemeColors = this.options.overrideThemeColors != null ? this.options.overrideThemeColors : this._getConfig('colors.overrideThemeColors');
  ...
  // --- SCROLL-MAP SERVICE ---
  consumeScrollMapService(scrollMapService) {
    this.scrollMapService = scrollMapService;
    const diffViewModule = this;
    const BaseLayer = scrollMapService.Layer;

    if (!BaseLayer) {
      console.warn('diff-view: scroll-map Layer class not available');
      return;
    }

    // Create a custom layer class for diff-view
    class DiffViewLayer extends BaseLayer {
      constructor(editor) {
      ...
      recalculate() {
        ...
        if (this.editor === editor1) {
          for (const chunk of chunks) {
            for (let row = chunk.oldLineStart; row < chunk.oldLineEnd; row++) {
              this.items.push({
                row: this.editor.screenPositionForBufferPosition([row, 0]).row,
                cls: 'removed'
              });
            }
          }
          ...
          for (const chunk of chunks) {
            for (let row = chunk.newLineStart; row < chunk.newLineEnd; row++) {
              this.items.push({
                row: this.editor.screenPositionForBufferPosition([row, 0]).row,
                cls: 'added'
              });
    ...
    scrollMapService.registerLayer('diff-view', DiffViewLayer);
